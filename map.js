...
$(function() {
    $("#layer").change(function() {
        ...
        $(document).ready(function() {
            $.ajax({
                type: "GET",
                url: "http://localhost:8085/geoserver/wfs?service=WFS&request=DescribeFeatureType&version=1.1.0&typeName=" + value_layer,
                dataType: "xml",
                success: function(xml) {
...
function query() {
    ...
    var url = "http://localhost:8085/geoserver/ows?service=WFS&version=1.0.0&request=GetFeature&typeName=" + value_layer + "&CQL_FILTER=" + value_attribute + "%20" + value_operator + "%20" + value_txt + "&outputFormat=application/json"
    ...
    if (value !== 'None') {
		...
		draw1 = new ol.interaction.Draw({
            ...
            draw1.on('drawend', function(evt) {
                ...
                var url = "http://localhost:8085/geoserver/wfs?request=GetFeature&version=1.0.0&typeName=" + value_layer + "&outputFormat=json&cql_filter=INTERSECTS(the_geom," + wkt + ")";
                //alert(url);
